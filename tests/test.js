...
describe('TEST EMAILS AGAINST VALIDATOR', () => {
  it('Should Be Valid', () => {
    validSupported.forEach(email => {
      try {
        ...
        console.log(`❌ FAILED: "${email}" should be valid but was rejected`)
        throw error
      }
    })

    try {
      expect(validate(validSupported)).to.equal(true)
    } catch (error) {
      console.log(`err ${error}`)
      throw error
    }
  ...
  it('Should Be Invalid', () => {
    invalidSupported.forEach(email => {
      try {
        ...
        console.log(`❌ FAILED: "${email}" should be invalid but was accepted`)
        throw error
      }
    })

    try {
      expect(validate(invalidSupported)).to.equal(false)
    } catch (error) {
      console.log(`err ${error}`)
      throw error
    }
  ...
  it('Should Be Invalid(UnSupported By Module)', () => {
    validUnsupported.forEach(email => {
      try {
        ...
        console.log(`❌ FAILED: "${email}" is unsupported and should be rejected but was accepted`)
        throw error
      }
    })
  })
  ...
  try {
    expect(validate(validUnsupported)).to.equal(false)
  } catch (error) {
    console.log(`err ${error}`)
    throw error
  }
